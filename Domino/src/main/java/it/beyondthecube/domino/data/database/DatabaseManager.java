...
public class DatabaseManager {
	...
	public void createResident(UUID u, String nick) throws DatabaseException {
		Utility.sendConsole("Attempting to create resident...");
		SQLQuery q = null;
		try {
			String[] params = { u.toString(), nick };
			q = new SQLQuery("insert into £.resident(idresident,nick) values(?,?);", params, msqlc);
			q.excecuteUpdate();
			Utility.sendConsole("Done!");
		} catch (SQLException e) {
			Utility.sendConsole("DB Error creating resident");
	...
	public ComLocation[] newArea(UUID id, Location<World> angle1, Location<World> angle2, Resident owner, City c,
		...
		try {
			...
			String[] params2 = { String.valueOf(angle2.getBlockX()), String.valueOf(angle2.getBlockY()),
					String.valueOf(angle2.getBlockZ()), game.getServer().getDefaultWorldName()};
	...
	public void updateResidentNickname(UUID u, String nick) throws DatabaseException {
		String[] params = {nick, u.toString()};
